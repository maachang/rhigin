...
rhiginによって、node-jsと違い「非同期実装が不要」で、最も楽で「手間がかからず」かつ「ファイル更新で自動リロード」なので、マイクロサービスを開発することが「とても楽で手間がかからず」に行えます。
...
ただ、現状の「rhino」のバージョンは「1.7.11」で「一部のES5」たとえば「let」などは利用可能です（constは利用できない）。  
(そのうち対応予定)
