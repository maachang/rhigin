...
public class ShutdownSignal {
	 ...
	 * シャットダウン送信内容.
	 */
	public static final byte[] SHUTDOWN_BINARY = { (byte) 0x76, (byte) 0x19, (byte) 0x10, (byte) 0xd0 };
	...
	public static boolean send(int port, int timeout) throws Exception {
		...
		s.send(new DatagramPacket(SHUTDOWN_BINARY, 0, SHUTDOWN_BINARY.length, InetAddress.getByName(LOCAL_ADDRESS),
				port));
		boolean ret = true;
		// 停止完了を待つ.
		try {
