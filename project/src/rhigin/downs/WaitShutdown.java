...
public class WaitShutdown {
	...
	public void exitShutdown() {
		if (srcPort > 0) {
			try {
				connection.send(
						new DatagramPacket(ShutdownSignal.SHUTDOWN_BINARY, 0, ShutdownSignal.SHUTDOWN_BINARY.length,
								InetAddress.getByName(ShutdownSignal.LOCAL_ADDRESS), srcPort));
			} catch (Exception e) {
			}
		}
	}
