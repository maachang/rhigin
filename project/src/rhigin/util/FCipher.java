...
public class FCipher {
	...
	private static final class CBase64 {
		...
		public static final byte[] decode(final String base64) {
			...
			for (i = 0, j = 0, k = 0; i < len; i++, j += 4, k += 3) {
				ret[k] = (byte)(((DEC_CD[base64.charAt(j)] & 0x0000003f) << 2) | ((DEC_CD[base64.charAt(j+1)] & 0x00000030) >> 4));
				ret[k + 1] = (byte)(((DEC_CD[base64.charAt(j+1)] & 0x0000000f) << 4) | ((DEC_CD[base64.charAt(j+2)] & 0x0000003c) >> 2));
				ret[k + 2] = (byte)(((DEC_CD[base64.charAt(j+2)] & 0x00000003) << 6) | (DEC_CD[base64.charAt(j+3)] & 0x0000003f));
			}
			switch (etc) {
				case 0:
					j = len * 4;
	...
	public static final String hashUuid(String s) {
		try {
			return Converter.byte16ToUUID(hash_raw(s.getBytes("UTF8")));
		} catch(Exception e) {
			throw new FCipherException(e);
		}
	}
	...
	public static final String hashUuid(final byte[] bin) {
		return Converter.byte16ToUUID(hash_raw(bin));
	}
	 ...
	 * @param h
	 */
	public FCipher(Xor128 rand, String h) {
	 ...
	 * @param h
	 */
	public FCipher setHead(String h) {
		this._head = h;
		return this;
	}
	 ...
	 * @return
	 */
	public FCipher setRand(Xor128 rand) {
		this._rand = (rand == null) ? new Xor128(System.nanoTime()) : rand;
		return this;
	}
	 ...
	 * @return
	 */
	public byte[] key(String word, String src) {
		if(src == null) {
			src = "-l_l-u_f-s_m-";
		}
	...
	public byte[] bdec(String value, byte[] pKey, String head) {
		...
		if( destStepNo != stepNo ) {
			throw new FCipherException("decode:Decryption process failed.");
		}
		return body;
	}
	...
	private static final byte[] code16(String s) {
		int n, i, j;
		byte[] ret = new byte[] {
			(byte)177, (byte)75, (byte)163, (byte)143, (byte)73, (byte)49, (byte)207, (byte)40,
			(byte)87, (byte)41, (byte)169, (byte)91, (byte)184, (byte)67, (byte)254, (byte)89
		};
		int len = s.length();
		for(i = 0; i < len; i ++) {
