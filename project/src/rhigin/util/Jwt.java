		...
		try {
			String value = new StringBuilder(JWT_HEADER).append(".")
					.append(Base64.encode(
							(payload instanceof String ? (String) payload : Json.encode(payload)).getBytes("UTF8")))
					.toString();
			return new StringBuilder(value).append(".").append(signature(key.getBytes("UTF8"), value.getBytes("UTF8")))
					.toString();
