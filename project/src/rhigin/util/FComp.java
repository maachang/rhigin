...
public class FComp {
	 ...
	 *         同様オブジェクトが返却されます.
	 */
	public static final FCompBuffer compress(byte[] in, int offset, int length, FCompBuffer out, int shift) {
		int lenM4 = length - 4;
	 ...
	 * @exception 例外.
	 */
	public static final FCompBuffer decompress(final byte[] in, final int offset, final int length, FCompBuffer out) {
		int p;
		...
		if (targetIndex > targetLength) {
			throw new FCompException("Superfluous input data encountered on offset (index:" + targetIndex + " max:"
					+ targetLength + ")");
		}
		return out;
	}
