...
public class FComp {
	 ...
	 */
	public static final FCompBuffer compress(byte[] in) {
	 ...
	 */
	public static final FCompBuffer compress(byte[] in, FCompBuffer out) {
	 ...
	 * @return compressed data block
	 */
	public static final FCompBuffer compress(byte[] in, int offset, int length) {
	 ...
	 * @return compressed data block
	 */
	public static final FCompBuffer compress(FCompBuffer in) {
	 ...
	 * @return reference to <code>out</code>
	 */
	public static final FCompBuffer compress(FCompBuffer in, FCompBuffer out) {
	 ...
	 */
	public static final FCompBuffer decompress(byte[] in) {
	 ...
	 * @return reference to <code>out</code>
	 */
	public static final FCompBuffer decompress(byte[] in, FCompBuffer out) {
	 ...
	 * @return decompressed data block
	 */
	public static final FCompBuffer decompress(byte[] in, int offset, int length) {
	 ...
	 * @return decompressed data block
	 */
	public static final FCompBuffer decompress(FCompBuffer in) {
	 ...
	 * @return reference to <code>out</code>
	 */
	public static final FCompBuffer decompress(FCompBuffer in, FCompBuffer out) {
	 ...
	 * @param out 圧縮結果を格納するBufferオブジェクトを設定します.
	 *			設定されない場合は、内部で新規作成されます.
	 * @param shift 圧縮テーブル長を増やす場合は、この値に整数を設定します.
	 * @return JSnappyBuffer 圧縮されたBufferオブジェクトが返却されます.
	 *				この情報は第４引数でオブジェクトを渡した場合、
	 *				同様オブジェクトが返却されます.
	 */
	public static final FCompBuffer compress(byte[] in, int offset, int length,
		FCompBuffer out,int shift) {
		int lenM4 = length - 4 ;
		...
		// 圧縮バッファの調整.
		if(out == null) {
			out = new FCompBuffer(calcMaxCompressLength( length ));
		}
		else {
		...
		for(int i = offset + 4; i < offLenM4; i++) {
			...
			if( ( n = ((in[i]&0xff)<<24)|((in[i+1]&0xff)<<16)|
				((in[i+2]&0xff)<<8)|(in[i+3]&0xff) ) ==
					( (( in[ fp = _cc[ ( ( n * HASH ) >> hashShift ) & _msk ] ]&0xff)<<24)|
				...
				if( in[fp+4] == in[i+4] ) {
					...
					// 圧縮対象同一までチェック.
					for( tLen = ( io + LIMIT < offLen ) ?
						( ( o + LIMIT < i ) ? LIMIT : i-o ) : offLen-io ;
						hLen < tLen && in[ o ++ ] == in[ io ++ ] ;
						hLen ++ ) ;
				}
				else {
					hLen = 4 ; // 圧縮連続長.
	 ...
	 * @param out 解凍結果を格納するBufferオブジェクトを設定します.
	 *			設定されない場合は、内部で新規作成されます.
	 * @return JSnappyBuffer 解凍されたBufferオブジェクトが返却されます.
	 *				この情報は第４引数でオブジェクトを渡した場合、
	 *				同様オブジェクトが返却されます.
	 * @exception 例外.
	 */
	public static final FCompBuffer decompress( final byte[] in, final int offset,
		final int length, FCompBuffer out) {
		int p ;
		...
		// 解凍データ長をセット.
		if (out == null) {
			out = new FCompBuffer( targetLength );
		}
		else {
		...
		while(sourceIndex < offLen && targetIndex < targetLength) {
			...
			if( ( bc = in[sourceIndex] & 3 ) == 0 ) {
				...
				if( (o = ( n = (in[sourceIndex++] >> 2) & 0x3f ) - 60) > -1 ) {
					for( o++,c = 1,n = (in[ sourceIndex ] & 0xff) ; c < o ;
						n |= ( in[ sourceIndex+c ] & 0xff ) << ( c * 8 ),c ++ ) ;
					sourceIndex += o ;
				}
