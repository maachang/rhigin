		...
		public final Object call(Context ctx, Scriptable scope, Scriptable thisObj, Object[] args) {
			...
			if(args.length >= 1 && args[0] instanceof byte[]) {
				if(args.length >= 3) {
					switch(type) {
					case 0:
						buf =FComp.compress((byte[])args[0], Converter.convertInt(args[1]), Converter.convertInt(args[2]));
						return buf.toByteArray();
					case 1:
						buf =FComp.decompress((byte[])args[0], Converter.convertInt(args[1]), Converter.convertInt(args[2]));
						return buf.toByteArray();
					}
